...
const multipleStoreData = async (itemArr) => {
    try {
        await AsyncStorage.setMany(itemArr)
        return true;
    } catch (e) {
        return false;
    }
...
const getMultipleData = async (items) => {
    try {
        const values = await AsyncStorage.getMany(items);
...
const multipleRemove = async (items) => {
    try {
        await AsyncStorage.removeMany(items);
        return true;
    } catch (e) {
        return false;
    }
...
export { clearStorage, getAllKeys, getData, getMultipleData, mergeItem, multipleRemove, multipleStoreData, singleRemove, storeData };
